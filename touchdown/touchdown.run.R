setwd("/Users/johnrivas/Desktop/UHD/SCHOOL/Fall_2017/Predictive/Project/GitHub/touchdown")
source("touchdown.models.R")
training <- read.csv("touchdown.train.csv")
testing <- read.csv("touchdown.test.csv")
training <- training[,-1]
testing <- testing[,-1]
training$Touchdown <- as.factor(training$Touchdown)
testing$Touchdown <- as.factor(testing$Touchdown)
...
########## Random Forest
# Train model
training$Touchdown <- make.names(training$Touchdown)
testing$Touchdown <- make.names(testing$Touchdown)
...
fit_logit <- glm(Touchdown ~ ., data = training, family = "binomial")
...
rankP[class > 0.5 ] = 1
rankP[class <= 0.5 ] = 0
accuracyTesting <- confusionMatrix( rankP, testing$Touchdown)
