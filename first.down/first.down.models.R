...
do.e071.DT <- function(training){
  ...
  dec_tree <- tune.rpart(FirstDown ~ . , data = training , minsplit = c(1,2),
...
{  print("random forest")
  ...
  rf.Fit <- train(FirstDown ~ .,data = training,method = "rf",metric = "Accuracy",preProc = c("center", "scale"),
...
do.RadialKernelSVM <- function(training)
{
  ...
  svm.Fit <- train(FirstDown ~ ., data= training,perProc = c("center", "scale"),
...
do.KNN <- function(training){
  ...
  knn_fit <- train(FirstDown ~ ., data = training,method = "kknn",metric = "Accuracy",perProc = c("center", "scale"),tuneGrid = grid,
                   trControl = ctrl.cross)
  knn_fit
}
...
do.CARET.Predict<-function(tune.model,test_data){
  ...
  cm<- confusionMatrix(Pred,test_data$FirstDown)
  cm
}
