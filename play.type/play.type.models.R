...
do.e071.DT <- function(training){
  ...
  dec_tree <- tune.rpart(PlayType2 ~ . , data = training , minsplit = c(1,2),
...
{  print("random forest")
  ...
  rf.Fit <- train(PlayType2 ~ .,data = training,method = "rf",metric = "Accuracy",preProc = c("center", "scale"),
...
do.RadialKernelSVM <- function(training)
{
  ...
  grid <- expand.grid(sigma = sigma , C = 2^seq(from=14,by = 1, to =16))
  print("Radial Kernel SVM")
  ctrl.cross <- trainControl(method = "cv", number = 5,classProbs = TRUE,savePredictions=TRUE)
  svm.Fit <- train(PlayType2 ~ ., data= training,perProc = c("center", "scale"),
...
{
  set.seed(1)
  grid <- expand.grid(scale = 1, degree = c(1), C = 2^seq(from=1,by = 1, to =1))
...
do.KNN <- function(training){
  ...
  knn_fit <- train(PlayType2 ~ ., data = training,method = "kknn",metric = "Accuracy",perProc = c("center", "scale"),tuneGrid = grid,
                   trControl = ctrl.cross)
  knn_fit
}
...
do.CARET.Predict<-function(tune.model,test_data){
  ...
  cm<- confusionMatrix(Pred,test_data$PlayType2)
  cm
}
